 ...
 */
exports.addSecondsToUser = function (req, res) {
  var user = req.user;
  var body = req.body;
  var remaingSec = body.remaingSec;

  if (user && remaingSec >= 0) {
    user = new User(req.user)
    user.totalFreeSeconds = remaingSec;
...
exports.getHistory = function (req, res) {
  ...
  if (user) {
    User.findOne({
      "_id": user._id
    }).populate("booksIssueHistory", ["title", "created"]).then(function (suc) {
      delete suc.password
      delete suc.salt
      res.status(200).send({
        "user": suc
...
exports.removeHistory = function (req, res) {
  var user = req.user;
  var book = req.body.book
  // console.log(req.body)

  if (user) {
    let booksIssueHistory = []
    for(let i of user.booksIssueHistory){
      if(i!=book._id){
        booksIssueHistory.push(i)
      }
    }
    user.booksIssueHistory = booksIssueHistory;
    user.save(function(){
      res.status(200).send({
        "message": "success"
      })
...
exports.me = function (req, res) {
  ...
  if (req.user) {
    safeUserObject = {
      ...
      totalFreeSeconds: req.user.totalFreeSeconds,
      booksIssueHistory:req.user.booksIssueHistory,
